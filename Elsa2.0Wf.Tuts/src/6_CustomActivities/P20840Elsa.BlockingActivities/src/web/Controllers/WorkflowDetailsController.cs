...
namespace Elsa.Server.Web.Controllers
{
    ...
    public class WorkflowDetailsController : Controller
    {
        ...
        {
            var workflowBluePrintList = await _workflowRegistry.ListAsync();

            var fileRecievedWorkflow = workflowBluePrintList.Where(wfBluePrint => !string.IsNullOrEmpty(wfBluePrint.Name) && wfBluePrint.Name.Contains("FileRecieved")).FirstOrDefault();

            var workflowDefIdSpec = new WorkflowDefinitionIdSpecification(fileRecievedWorkflow.Id);
        ...
        //{
        //    var workflowDefIdSpec = new WorkflowDefinitionIdSpecification(nameof(RegistrationWorkflow));
        ...
        public async Task<ActionResult<WfInstanceDetails>> GetWorkflowDetails()
        {
            ...
            var fileRecievedWorkflow = workflowBluePrintList.Where(wfBluePrint => !string.IsNullOrEmpty(wfBluePrint.Name) && wfBluePrint.Name.Contains("FileReceived")).FirstOrDefault();
