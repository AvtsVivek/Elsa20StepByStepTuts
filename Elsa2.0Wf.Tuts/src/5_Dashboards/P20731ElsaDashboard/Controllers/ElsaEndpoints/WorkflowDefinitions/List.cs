...
namespace P20731ElsaDashboard.Controllers.ElsaEndpoints.WorkflowDefinitions
{
    [ApiController]
    //[ApiVersion("1")]
    ...
    public class List : Controller
    {
        ...
        public async Task<ActionResult<PagedList<WorkflowDefinitionSummaryModel>>> Handle(int? page = default, int? pageSize = default, VersionOptions? version = default, CancellationToken cancellationToken = default)
        {
            ...
            var endpointString = QueryHelpers.AddQueryString("/v1/workflow-definitions/", query!);
            var response = await client.GetAsync(endpointString);
            var jsonResponseString = await response.Content.ReadAsStringAsync();
            var workflowDefs = JsonConvert.DeserializeObject<PagedList<WorkflowDefinitionSummaryModel>>(jsonResponseString);
